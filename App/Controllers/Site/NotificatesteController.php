...
class NotificartesteController extends BaseController {
    ...
    public function cadastrar(){
        ...
        if(isset($_POST['submit'])){
                            ...
                            for($i=0; $i<$quantidade_usuario; $i++) {
                                                                    ...
                                                                    */
                                                                    $this->CalculoDescontoAuxilios($objOs->id, $usuario->id,$hora_inicio_missao,$idaTrecho[0],$voltaTrecho[$quantidade_roteiro-1]);
...
*/
    ...
    public function CalculoDescontoAuxilios($id_ordem_servico,$cpf,$hora_inicio_missao,$idaPeriodo,$voltaPeriodo){
		...
		$Obj_ordem_servico->auxilioAlimentacao = $calculoAuxilio[0];
		$Obj_ordem_servico->auxilioTransporte = $calculoAuxilio[1];

		$Obj_ordem_servico->diasUteisAuxAlimentacao += $calculoAuxilio[2];
		
		$Obj_ordem_servico->diasUteisAuxTransporte += $calculoAuxilio[3];
	...
	//funcao para calcularAuxilio		
		public function calcularAuxilio($cpf, $diasUteis, $descontoAuxilio){
				...
				foreach (usuario::UsuarioCpf($cpf) as $usuario){
				
				$valorMensalAlimentacaoUsuario = $usuario->auxilio_alimentacao;
				
				$valorMensalTransporteUsuario = $usuario->auxilio_transporte;
